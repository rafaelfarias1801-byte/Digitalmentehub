...
export default function Dashboard({ profile }: Props) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setTotalVencido(unpaidTotal.filter(p => p.due_date && p.due_date < todayStr).reduce((s, p) => s + Number(p.amount), 0));
      setUpcomingSum(unpaidTotal.filter(p => p.due_date && p.due_date >= todayStr && p.due_date <= in7DaysStr).reduce((s, p) => s + Number(p.amount), 0));

      setUpcomingFinance(unpaidTotal
          .filter(p => p.due_date && p.due_date <= in7DaysStr)
          .sort((a, b) => a.due_date.localeCompare(b.due_date))
          .slice(0, 5) 
          .map(p => ({
            id: p.id, amount: p.amount, due_date: p.due_date, description: p.description || "Fatura",
            caseName: caseMap[p.case_id] ?? "Agência", overdue: p.due_date < todayStr
          }))
      );
  ...
  if (loading) return (<div className="ws-page"><div style={{ color: "var(--ws-text3)", fontFamily: "Poppins", fontSize: ".8rem", marginTop: 40 }}>Carregando...</div></div>);

  return (
    <div className="ws-page" style={{ paddingBottom: isMobile ? 80 : 20 }}>
      <div className="ws-page-title" style={{ marginBottom: 2 }}>Olá{firstName ? `, ${firstName}` : ""}<span className="ws-dot">.</span></div>
      <div className="ws-page-sub" style={{ marginBottom: 12 }}>{new Date().toLocaleDateString("pt-BR", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</div>

      <div className="ws-stats" style={{ marginBottom: 12 }}>
        <div className="ws-stat" style={{ "--c": tasksOverdue > 0 ? "#ff5c7a" : "#e91e8c", ...compactCardStyle } as any}>
          <div className="ws-stat-label">Tarefas abertas</div>
          <div className="ws-stat-value" style={compactValueStyle}>{tasksPending}</div>
          <div className="ws-stat-detail" style={{ color: tasksOverdue > 0 ? "#ff5c7a" : "var(--ws-text3)" }}>{tasksOverdue > 0 ? `⚠ ${tasksOverdue} atrasada${tasksOverdue > 1 ? "s" : ""}` : `${tasksDone} concluídas`}</div>
        </div>
        <div className="ws-stat" style={{ "--c": totalVencido > 0 ? "#ff5c7a" : "#ffd600", ...compactCardStyle } as any}>
          ...
          <div className="ws-stat-detail" style={{ color: totalVencido > 0 ? "#ff5c7a" : (upcomingSum > 0 ? "#ffd600" : "var(--ws-text3)") }}>{totalVencido > 0 ? `⚠ ${fmt(totalVencido)} vencido` : (upcomingSum > 0 ? `⏳ ${fmt(upcomingSum)} nos próx. 7 dias` : "Em dia 🎉")}</div>
        </div>
        <div className="ws-stat" style={{ "--c": "#4dabf7", ...compactCardStyle } as any}>
        ...
        <div className="ws-stat" style={{ "--c": "#00e676", ...compactCardStyle } as any}>
      ...
      <div className="ws-cols2" style={{ marginBottom: 12 }}>
        <div className="ws-card">
          <div className="ws-card-title">Tarefas abertas
          ...
          <div className="ws-tasks">
            {recentTasks.length === 0 ? <div style={{ color: "var(--ws-text3)", fontSize: ".82rem" }}>Tudo em dia 🎉</div> : recentTasks.map(t => (
                <div key={t.id} className="ws-task" style={{ padding: "8px 10px" }}>
                  <div className="ws-check" onClick={() => handleCompleteTask(t.id)} style={{ cursor: "pointer" }} />
                  <div className="ws-task-text">{t.title}{t.related_name && <span style={{ fontSize: ".7rem", color: "var(--ws-text3)", marginLeft: 6 }}>— {t.related_name}</span>}</div>
          ...
          {recentApprovals.length === 0 ? <div style={{ color: "var(--ws-text3)", fontSize: ".82rem" }}>Nenhuma atividade ainda.</div> : <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {recentApprovals.map((a, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "var(--ws-surface2)", borderRadius: 10, borderLeft: `3px solid ${APPROVAL_COLOR[a.status] ?? "#8d97bb"}` }}>
                  ...
                  <span style={{ fontSize: ".75rem", fontWeight: 700, color: APPROVAL_COLOR[a.status], flexShrink: 0 }}>{APPROVAL_ICON[a.status]}</span>
                </div>
              ))}
            </div>}
        </div>
      ...
      <div className="ws-cols2">
        <div className="ws-card">
          ...
          {nextEvents.length === 0 ? <div style={{ color: "var(--ws-text3)", fontSize: ".82rem" }}>Nenhum evento próximo.</div> : <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {nextEvents.map((ev, i) => {
                const d = new Date(ev.date + "T12:00:00");
                const isToday = ev.date === new Date().toISOString().slice(0, 10);
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 14px", background: "var(--ws-surface2)", borderRadius: 10, borderLeft: `3px solid ${EVENT_COLOR[ev.type] ?? "var(--ws-accent)"}` }}>
                    <div style={{ flexShrink: 0, textAlign: "center", minWidth: 36 }}>
                      ...
                      <div style={{ fontFamily: "Poppins", fontSize: ".45rem", letterSpacing: "1px", color: "var(--ws-text3)", textTransform: "uppercase" }}>{d.toLocaleDateString("pt-BR", { weekday: "short" })}</div>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: ".85rem", fontWeight: 600, color: "var(--ws-text)", overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>{ev.title}</div>{isToday && <div style={{ fontSize: ".62rem", fontFamily: "Poppins", color: "var(--ws-accent)" }}>HOJE</div>}</div>
                  </div>
                );
              })}
            </div>}
        ...
        <div className="ws-card">
          ...
          {upcomingFinance.length === 0 ? <div style={{ color: "var(--ws-text3)", fontSize: ".82rem" }}>Tudo limpo! Nenhuma cobrança pendente. 🎉</div> : <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {upcomingFinance.map(f => (
                <div key={f.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 14px", background: "var(--ws-surface2)", borderRadius: 10, borderLeft: `3px solid ${f.overdue ? '#ff5c7a' : '#ffd600'}` }}>
                  <div style={{ flex: 1, minWidth: 0, paddingRight: 10 }}>
                    <div style={{ fontSize: ".85rem", fontWeight: 600, color: "var(--ws-text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{f.description}</div>
                    <div style={{ fontSize: ".68rem", color: "var(--ws-text3)", fontFamily: "Poppins", marginTop: 1 }}>{f.caseName} · {f.due_date.split('-').reverse().join('/')}</div>
                  </div>
                  <div style={{ fontSize: ".85rem", fontWeight: 800, color: f.overdue ? '#ff5c7a' : "var(--ws-text)", flexShrink: 0 }}>{fmt(f.amount)}</div>
                </div>
              ))}
            </div>}
        </div>
