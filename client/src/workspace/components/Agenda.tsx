...
export default function Agenda({ profile }: Props) {
  ...
  function openEdit(ev: CalEvent) {
    setEditing(ev);
    setForm({ title: ev.title, type: ev.type, client: ev.client ?? "", note: ev.note ?? "" });
    setModal(true);
  }
  ...
  async function saveEvent() {
    ...
    if (editing) {
      const { data, error } = await supabase
        .from("events")
        .update({ ...form, client: form.client || null, note: form.note || null })
        .eq("id", editing.id).select().single();
      if (!error && data) setEvents(prev => prev.map(e => e.id === editing.id ? data : e));
    } else {
      const { data, error } = await supabase
        .from("events")
        .insert({ ...form, date: selected, client: form.client || null, note: form.note || null })
        .select().single();
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20, alignItems: "start" }}>
        ...
        <div className="ws-card">
          ...
          <div className="ws-cal-grid">
            ...
            {prevPad.map((d, i) => (
              <div key={`p${i}`} className="ws-cal-day other">
            ...
            {currDays.map(d => {
              ...
              return (
                <div
                  key={d}
                  className={`ws-cal-day ${isToday ? "today" : ""}`}
                  style={{
                    cursor: "pointer",
                    background: isSel ? "#e91e8c22" : undefined,
                    borderColor: isSel ? "var(--ws-accent)" : undefined,
                    position: "relative",
                  }}
  ...
  {selectedEvents.length} evento{selectedEvents.length !== 1 ? "s" : ""}
            ...
            {selectedEvents.length === 0 ? (
              <div style={{ color: "var(--ws-text3)", fontSize: ".82rem", padding: "12px 0", textAlign: "center" }}>
                Nenhum evento.<br/>
                <span style={{ fontSize: ".75rem" }}>Duplo clique no calendário para adicionar.</span>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {selectedEvents.map(ev => (
                  <div key={ev.id} style={{
                    background: "var(--ws-surface2)", borderRadius: "var(--ws-radius-sm)",
                    padding: "12px 14px", borderLeft: `3px solid ${TYPE_COLORS[ev.type]}`,
                  }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                          <span style={{
                            fontFamily: "DM Mono", fontSize: ".55rem", letterSpacing: "1px",
                            textTransform: "uppercase", padding: "2px 7px", borderRadius: 4,
                            background: TYPE_COLORS[ev.type] + "22", color: TYPE_COLORS[ev.type],
                          }}>{TYPE_LABELS[ev.type]}</span>
                          {ev.client && ev.client !== "—" && (
                            <span style={{ fontSize: ".72rem", color: "var(--ws-text3)" }}>{ev.client}</span>
                          )}
                        </div>
                      ...
                      <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                        <button onClick={() => openEdit(ev)} style={{
                          background: "var(--ws-surface3)", border: "none", borderRadius: 6,
                          color: "var(--ws-text2)", cursor: "pointer", padding: "4px 8px", fontSize: ".75rem",
                        }}>✎</button>
                        <button onClick={() => deleteEvent(ev.id)} style={{
                          background: "var(--ws-surface3)", border: "none", borderRadius: 6,
                          color: "var(--ws-accent)", cursor: "pointer", padding: "4px 8px", fontSize: ".75rem",
                        }}>×</button>
                      </div>
                    </div>
                  </div>
          ...
          <div className="ws-card">
            ...
            {events
              ...
              .map(ev => (
                <div key={ev.id} style={{
                  ...
                  padding: "8px 0", borderBottom: "1px solid var(--ws-border)",
                  cursor: "pointer",
            ...
            {events.filter(e => e.date >= today).length === 0 && (
              <div style={{ color: "var(--ws-text3)", fontSize: ".8rem" }}>Nenhum evento próximo.</div>
            )}
          </div>
        </div>
      </div>
