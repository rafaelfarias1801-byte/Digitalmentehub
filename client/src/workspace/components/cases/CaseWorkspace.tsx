...
export default function CaseWorkspace({
  ...
  // Se a aba salva ficou oculta (ex: cliente acessou depois de admin), redireciona
  useEffect(() => {
    const visible = visibleTabs.map(t => t.id);
    if (!visible.includes(activeTab)) {
      setActiveTab(visible[0] ?? "calendario");
    }
  }, [profile?.role]);
  ...
  const renderTabContent = () => (
    <>
      {activeTab === "calendario"  && <TabCalendario caseData={caseData} profile={profile} readonly={isClient || isDesigner} />}
      {activeTab === "conteudo"    && <TabConteudo   caseData={caseData} profile={profile} readonly={isClient} />}
      {activeTab === "financeiro"  && <TabFinanceiro caseData={caseData} readonly={isClient || isDesigner} />}
      {activeTab === "contratos"   && <TabDocumentos caseData={caseData} type="documento" readonly={isClient || isDesigner} canUpload={false} />}
        ...
        <div style={{ background: "var(--ws-surface)", borderBottom: "1px solid var(--ws-border)", position: "sticky", top: 0, zIndex: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 16px 6px" }}>
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
              {caseData.logo_url
                ? <img src={caseData.logo_url} alt={caseData.name} style={{ width: 22, height: 22, borderRadius: 4, objectFit: "cover", flexShrink: 0 }} />
                : <div style={{ width: 22, height: 22, borderRadius: 4, flexShrink: 0, background: `linear-gradient(135deg,${caseData.color}44,${caseData.color}22)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: ".55rem", fontWeight: 800, color: caseData.color }}>{caseData.name.slice(0, 2).toUpperCase()}</div>
              }
