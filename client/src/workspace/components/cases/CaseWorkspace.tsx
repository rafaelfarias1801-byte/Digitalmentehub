        ...
        >
          <div
            style={{
              ...
              borderRadius: 8,
              overflow: "hidden",
              background: caseData.logo_url
                ? undefined
                : `linear-gradient(135deg,${caseData.color}44,${caseData.color}22)`,
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              marginBottom: 8,
          ...
          <span
            className={`ws-case-status ${STATUS_STYLES[caseData.status]}`}
            style={{
              marginTop: 4,
              display: "inline-block",
              color:
                caseData.status === "ativo"
                  ? "#00e676"
      ...
      <div style={{ flex: 1, overflow: "auto", background: "var(--ws-bg)" }}>
        ...
        <div style={{ padding: "28px 32px" }}>
          <div style={{ marginBottom: 24 }}>
            ...
            >
              {SUB_TABS.find((tab) => tab.id === activeTab)?.icon}{" "}
              {SUB_TABS.find((tab) => tab.id === activeTab)?.label}
          ...
          {activeTab === "calendario" && (
            <TabCalendario caseData={caseData} profile={profile} />
          )}

          {activeTab === "conteudo" && (
            <TabConteudo caseData={caseData} profile={profile} />
          )}

          {activeTab === "financeiro" && (
            <TabFinanceiro caseData={caseData} />
          )}

          {activeTab === "contratos" && (
            <TabDocumentos caseData={caseData} type="contrato" />
          )}

          {activeTab === "documentos" && (
            <TabDocumentos caseData={caseData} type="documento" />
          )}

          {activeTab === "notas" && (
            <TabNotas caseData={caseData} profile={profile} />
          )}
        </div>
      </div>
    </div>
